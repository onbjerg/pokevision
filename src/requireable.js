...
const fetch = (lat, lng, filter = null, jobId = null) => {
	return axios.get(`${baseURI}/data/${lat}/${lng}/${jobId}`)
		.then( (res) => {
...
const scan = (lat, lng, filter = null) => {
	return axios.get(`${baseURI}/data/${lat}/${lng}`)
		.then( (res) => {
			...
			return promiseRetry( (retry, tries) => {
				return fetch(lat, lng, filter, jobId)
					.catch( (err) => retry(err) );
			});
		});
};
